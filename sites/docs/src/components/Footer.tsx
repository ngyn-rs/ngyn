...
function SocialLink({
    ...
    return (
        <Link href={href} className="group">
            ...
            <Icon className="h-5 w-5 fill-zinc-700 transition light:group-hover:fill-zinc-900 group-hover:fill-zinc-500" />
        </Link>
    );
}
...
function SmallPrint() {
    return (
        <div className="flex flex-col items-center justify-between gap-5 border-t border-zinc-900/5 pt-8 sm:flex-row dark:border-white/5">
            <p className="text-xs light:text-zinc-600 dark:text-zinc-400">
                &copy; 2023 - {new Date().getFullYear()}{" "}
                <a href="https://elcharitas.wtf">Jonathan Irhodia</a>.
            </p>
            <div className="flex gap-4">
                {/* <SocialLink href="#" icon={XIcon}>
					Follow us on X
				</SocialLink> */}
                <SocialLink href="https://github.com/ngyn-rs" icon={StackoverflowIcon}>
                    StackOverflow
                </SocialLink>
                <SocialLink href="https://github.com/ngyn-rs" icon={GitHubIcon}>
                    Follow us on GitHub
                </SocialLink>
            </div>
        </div>
...
export function Footer() {
    return (
        <footer className="container w-full space-y-10 pb-16">
            <SmallPrint />
        </footer>
    );
}
